...
**Engram is a code reviewer that learns how *you* code from your git history — and catches the mistakes you personally keep making, before you ship them again.**
...
2. **0:30–1:30** — the loop: how it learned them (`engram learn` over real history) → the memory that fired, its salience climbing across 3 commits → it forgetting a stale preference. The number keeps climbing.
3. **1:30–2:00** — the thesis: every other AI tool forgets you the moment the session ends. Engram is the first one that *accumulates* a model of how you, specifically, code. The benchmark proves the engine. MIT, on Alibaba Cloud.
