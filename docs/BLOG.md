...
- **Forget** — one-off noise decays below a floor; a stale decision gets *superseded* by a newer
  one, with an audit trail. (You switched Redux → Zustand? The old note steps aside.)
...
can never fabricate); Qwen reads the specific code, decides whether it's a real bug *in this
context*, writes a concrete fix, and can even *decline to warn* on a false positive — the reasoning
a regex or a linter can't do.
...
packing under a token budget, contradiction audit trails, turning a repeat mistake into a rising
signal — live in the engine *around* the model, not in the model call itself.
...
it — and stops you before you ship. The moat isn't the detector (that's swappable — a better Qwen
